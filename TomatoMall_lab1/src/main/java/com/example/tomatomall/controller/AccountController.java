...
public class AccountController {
    ...
    public Response<AccountVO> getAccountInfo(@RequestHeader(value = "token", required = false) String token, @PathVariable String username) {
        ...
        // 5. 通过用户名查询目标用户的信息
        return Response.buildSuccess(accountService.getAccountByUsername(username));
    }
     ...
     */
    @PostMapping()
    public Response<Boolean> createAccount(@RequestBody AccountVO accountVO) {
     ...
     */
    @PutMapping()
    public Response<Boolean> updateAccount(@RequestBody AccountVO accountVO) {
